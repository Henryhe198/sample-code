...
require(['vue', 'booksMethod'], function(vue, bk) {
	...
	if(mui.os.ios) {
		$("header").css({ "padding-top": "20px", "height": "174px !important"});
		$(".mui-content").css("padding-top", "174px");
		$(".ct-tab").css("top", "24px");
	}
	$('body').show();
	//判断网络状态
	...
	var interest = {
		data : [],			   // 存放数据源
		interestRate : 0.0500, // 年利率 默认值美元活期0.0500%
		state: 0, 			   // 定活期状态
		$currency : $("#currency"),
		$saveType : $("#saveType"),
		init : function(){
			var that = this;
			that.getBankInterest();
			that.getLocalStorage();
			$("#total,#days").on("input propertychange",function(){
				var $this = $(this);
				$this.val($this.val().replace(/[^\d]/g,''));
				that.toCalculate();
			})
			$(document).on("hEvent",function(){//触发自定义事件——币种
				that.getInterestRate();
			})
			$(document).on("sEvent",function(){//触发自定义事件——储蓄类型
				that.getInterestRate();
			})
			$(".ct-tab-title").on("tap","li",function(){
				that.tabSelect($(this));
				that.getInterestRate();
			})
			
			$("#interestTable").on("tap",function(){
				$("input").blur();
				if(vm.onLine){
					//打开页面
					mui.openWindow({
					    url: 'interestTable.html?currencyType=' + that.$currency.val(), 
						id:'interestTable'
					});
				}else{
					mui.toast("网络未连接，请检查网络！")
		...
		getLocalStorage : function(){
			var state = localStorage.getItem("state") || 0,
				currencyVal = localStorage.getItem("currencyVal") || 1,
				currencyText = localStorage.getItem("currencyText") || "美元",
				saveType = localStorage.getItem("saveType") || 2,
				saveTypeText = localStorage.getItem("saveTypeText") || "7天通知",
				days = localStorage.getItem("days") || 7;
			this.tabSelect($(".ct-tab-title").children().eq(state));
			this.$currency.val(currencyVal);
			this.$saveType.val(saveType);
			this.$currency.next("em").text(currencyText);
			this.$saveType.next("em").text(saveTypeText);
			this.$saveType.parent(".saveType").data("days",days);
			this.getInterestRate();
		},
		// 设置缓存数据
		setLocalStorage : function(){			
			localStorage.setItem("state",this.state);
			localStorage.setItem("currencyVal",this.$currency.val());
			localStorage.setItem("saveType",this.$saveType.val());
			localStorage.setItem("currencyText",this.$currency.next("em").text());
			localStorage.setItem("saveTypeText",this.$saveType.next("em").text());
			localStorage.setItem("days",this.$saveType.parent(".saveType").data("days"));
		},
		// 选项卡的切换
		tabSelect : function($this){
			var index = $this.index(),$typeBox = $("#typeBox"),$daysBox = $("#daysBox");
		...
		getInterestRate : function(){
			...
			var $rate = $(".rate");
			if(!that.state){
				var currencyType = that.$currency.val(),durationType = 1;
			}else{
				var currencyType = that.$currency.val(),durationType = that.$saveType.val();
			}
			var url = Consts.OKRUrl + 'api/FXFinanceDepoRateApi?currencyType='+ currencyType +'&durationType=' + durationType;
			mui.ajax(url, {
				data: {},
				...
				success: function(data) {
					if(data.code == 0) {
	                    ...
	                    if (dataCent) {
							that.interestRate = dataCent.DepoRate;
							$rate.text(that.interestRate + "%");
							that.toCalculate();
						}
					}else{
						mui.toast('计算失败');
		...
		// 遍历数据 获取年利率 本地数据
		getInterestRate1 : function(){
			if(this.data.length){
				var currencyVal = this.$currency.val(),$rate = $(".rate"),item = this.data;
				if(currencyVal){
					for(var i in item){
						if(item.hasOwnProperty(i)){
							if(item[i].type == currencyVal){
								if(this.state){
									var o = item[i].fixed,saveTypeVal = this.$saveType.val();
									for(var x in o){
										if(o[x].saveType == saveTypeVal){
											this.interestRate = o[x].rates;
											$rate.text(o[x].rates + "%");
											this.toCalculate();
											return false;
										}
									...
									this.interestRate = item[i].rates;
									$rate.text(item[i].rates + "%");
									this.toCalculate();
									return false;
								}
							}
		...
		toCalculate : function(){
			var totalVal = $("#total").val(),daysVal = $("#daysBox").css("display") != "none"?$("#days").val():$(".saveType").data("days"),interest = 0,principalI = 0;	
			if(totalVal && daysVal){
				interest = (totalVal * daysVal * (this.interestRate / 100) / 360).toFixed(3); // 利息 = 本金 * 天数 * 利率 / 360
				principalI = (parseFloat(totalVal) + parseFloat(interest)).toFixed(3); // 本息 = 本金 + 利息
				$("#interest").text(interest);
				$("#principalI").text(principalI);
			}	
		},
		//获取利率表
		getBankInterest : function(){
			var that = this;
			var url = '/js/gulp/json/bankInterestRates.json';
			mui.ajax(url, {
				data: {},
				dataType: 'json', //服务器返回json格式数据
				type: 'get', //HTTP请求类型
				...
				success: function(data) {
					if(data) {
						//that.data = data.current;
						var picker = new mui.PopPicker();
						var pickerType = new mui.PopPicker();
						picker.setData(data.currencyData);
						pickerType.setData(data.fixedType);
						$(".currency").on("tap",function(){
							$("input").blur();
							var $this = $(this);
							picker.show(function (selectItems) {
								$this.find("em").text(selectItems[0].text);
								$this.find("input").val(selectItems[0].currencyType);
								$(document).trigger('hEvent', selectItems[0].currencyType);//自定义事件--选择币种触发
							})
						})
						$(".saveType").on("tap",function(){
							var $this = $(this);
							pickerType.show(function (selectItems) {
								$this.find("em").text(selectItems[0].text);
								$this.find("input").val(selectItems[0].saveType);
								$this.data("days",selectItems[0].days)
								$(document).trigger('sEvent', selectItems[0].saveType);//自定义事件--选择类型触发
							})
						})
					} else {
						vm.currencyData = "no";
