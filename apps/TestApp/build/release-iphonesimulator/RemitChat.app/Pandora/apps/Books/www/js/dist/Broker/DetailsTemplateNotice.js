...
return {
	...
	require(['ImageMethod','imgloads'], function (im, imgLoad) {
	    ...
	    $('body').on('tap', '.Related_Centent', function () {
	        $this = $(this);
	        window.location.href = '/News/NoticeTemplate?id=' + $this.data('id');
	    })
	    $(function () {
	        //function fhFn_close() {
	    ...
	    $.ajax({
	        type: 'GET',
	        url: Consts.OKRUrl148 + 'api/ANNCMobileApi?Id=' + WinUrlId,
	                ...
	                if (data.bodyMessage && data.bodyMessage != "null") {
	                    ...
	                    if (dataCen.More) {
	                        ...
	                        for (var i = 0; i < DataMore.length; i++) {
	                            News += '<div class="Related_Centent" data-id="'+ DataMore[i].Id + '">';
	                            News += '<a href="/News/NoticeTemplate?id=' + DataMore[i].Id + '">';
...
define("DetailsTemplateNotice", function(){});
