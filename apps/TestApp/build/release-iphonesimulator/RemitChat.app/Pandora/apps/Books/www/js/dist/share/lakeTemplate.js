require(['vue', 'IsLoadMethod'], function (vue, IsLoadMethod) {
...
//	}).show();
	...
	WinUrlId = GetQueryString('id');
    UsercurId = GetQueryString('curuser');
    UserUrlid = GetQueryString('user');		
//	mui.init({
//		pullRefresh: {
                                ...
                                if (datapagePic.length > 0) {
                                    for (var i = 0; i < datapagePic.length; i++) {
                                        if (datapagecentent.indexOf('{img' + i + '}') >= 0) {
                                            datapagecentent = datapagecentent.replace('{img' + i + '}', '<img src="' + datapagePic[i].SmallPic + '"data-preview-src="" data-preview-group="1">');
                                        }
