...
require(['vue','booksMethod'], function(vue,bk) {
	...
	$("body").on("tap","#share",function(){
		if(vm.shareDate){
			if(type == 1){
				bk.booksInit("booksShare");
			}else{
				var id = vm.shareDate.EvalTraderId,
				sContent = vm.shareDate.MContent.Content,
				sImgUrl = vm.shareDate.BrokerExtenInfo.Logo;
				var obj = {
					'id':id,
					'sContent':sContent,
					'sImgUrl':sImgUrl
	...
	$('body').on('tap', '.Related_Centent', function() {
		if(vm.onLine){
			mui.openWindow({
				id:'reviewTemplate',
				url: 'reviewTemplate.html?type=2&id=' + $(this).data('id'),
				extras: {}
			});
		}else{
			...
			success: function(data) {
				if(data.code == 0) {
					if(data.bodyMessage) {
						...
						vm.shareDate = dataCen;
						vm.content = type == "1" ? mainDetails(dataCen) : listDetails(dataCen);
						vm.moreReview = type == "1" ? dataCen.BrokerEvalTrader : dataCen.More;
						mui.previewImage();
					} else {
						mui.toast('暂无数据');
					}
	...
	function typeUrl(type) {
		switch(parseInt(type)) {
			case 1:
				var url = Consts.OKRUrl148 + 'api/BrokerEvalPeriodApi_Mobile/' + id;
				break;
			case 2:
				var url = Consts.OKRUrl148 + 'api/BrokerEvalTraderApi_Mobile?evalTraderId=' + id
				break;
			default:
				break;
		}
	...
	function mainDetails(data) {
		var htmlContent = "";
		htmlContent += '<div id="details_content"><div class="details_title"><h2>' + data.PeriodName + '</h2>';
			...
			if(UrlObj.length > 0) {
				for(var i = 0; i < UrlObj.length; i++) {
					if(DateContent.indexOf('{img' + i + '}') >= 0) {
						DateContent = DateContent.replace('{img' + i + '}', '<img src="' + UrlObj[i].Big + '"data-preview-src="" data-preview-group="1">');
					}
