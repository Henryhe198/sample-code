...
require([], function () {
    ...
    $(".rank-but").on("tap", function () {
        ...
        postComments(tradeUserId);
    })
    
    var postComments = function (tradeUserId) {
        var comment = $("#comment").val().trim();
        if (validation(comment)) {
            $.ajax({
                type: 'post',
                url: Consts.OKRUrl148 + 'api/CommentApi',
                data: { type: 7, aboutId: tradeUserId, Uid: uId, Ip: Ip, content: comment },
                dataType: 'json',
...
define("commentsSubmit", function(){});
