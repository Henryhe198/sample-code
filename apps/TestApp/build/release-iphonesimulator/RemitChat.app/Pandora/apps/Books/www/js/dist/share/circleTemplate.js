...
require(['vue', 'IsLoadMethod','molieInit'], function (vue, IsLoadMethod,init) {
	...
	$.ajax({
		type: 'GET',
		url:Consts.OKRUrl + 'api/FXMicroBlogMsgApi?UserId=' + UserId + '&MessageId=' + msgId,
		data: {},
		cache:true,
		dataType: 'json',
		success: function(data) {
			if(data.code == 0) {
				if(data.bodyMessage && data.bodyMessage != "null") {
					...
					var dataCom = dataCen.Commits;
					//var dataMP = dataCen.MainPost[0];
					var dates = new Date(dataCen.CreateTime);
                    var Time_Month = dates.getMonth() + 1;
                    var Time_Date = dates.getDate();
                    
                    //分享内容加载
					if(dataCen.CreateTime && dataCen.UserName && dataCen.MsgContent) {	
						html +='<div class="ct-name-time"><span>' + Time_Month +'月'+ Time_Date +'日'+ '</span><span>'+ dataCen.UserName +'</span></div>';
                       	//dataMcon = eval('(' + dataMP.Mcontent + ')');
                        
						html += '<p>' + dataCen.MsgContent + '</p>';
						vm.share = html;
						//mui.previewImage();
                    ...
                    //评论加载
                    for (var t = 0; t < dataCom.length; t++) {
                    	comment +='<div class="comment_read"><div class="comment_img"><img src="' + dataCom[t].HeadIcon + '"></div>';
                    	comment +='<div class="comment"><h3 class="name">'+ dataCom[t].ReplyUserName + '</h3>';
                        
                        comment += '<p class="activeshow"><span>' + dataCom[t].CommitContent + ' </span></p><div class="commetn_icon_show"><i class="mui-icon mui-icon-arrowdown"></i></div></div></div>'
...
define("circleTemplate", function(){});
