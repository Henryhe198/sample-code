...
require(['vue','booksMethod'], function(vue,bk) {
	...
	var constens = {
		...
		//实例化要使用的ajax数据，并赋值到 sm comment 两个空数组里面。
	var vm = new vue({
		el: '.rank',
		data: {
			sm: [],
	...
	initTradePlatformUrl();
	postrankModel(3);
	if(GetQueryString("uId")) {
		localStorage.setItem("uId", GetQueryString("uId"));
	}
	...
	$(".ct-tab-title").on("tap","li",function(){
		var index = $(this).index(),authStatus = 1;
		index == 0 ? authStatus = 3 : authStatus = 1;
		$(this).addClass("cur").siblings().removeClass("cur");
		constens.type = authStatus;
		constens.pageIndex = 1;
		postrankModel(authStatus,-1)
	})
	...
	//页面跳转
	$("body").on("tap", '.rank-box', function() {
		$this = $(this).children(".rank-title");
		if(vm.onLine){
			//打开页面
			mui.openWindow({
			    url: 'trader.html?tradeUserId=' + $this.data("id"), 
				id:'trader'
			});
		}else{
			mui.toast("网络未连接，请检查网络！")
	...
	//手指从屏幕移开时触发
	$('body').on('touchend', '.rank-box', function() {
		$(this).css({
			'background': '#fff'
		});
	})
	...
	//触摸排行列表时触发背景色改变
	$('body').on('touchstart', '.rank-box', function() {
		$(this).css({
			'background': '#f7f7f7'
		});
	})
	...
	function pullUpAction() {		
		constens.pageIndex++;
		postrankModel(constens.type,1);		
	}

	/*初始化提取交易平台Json文件*/
	function initTradePlatformUrl() {
		$.ajax({
			type: "get",
			...
			success: function(data) {
				constens.platformData = data.tradePlatform;

			}
		})
	}
	/*获取所有用户统计信息*/
	function postrankModel(authStatus,empty) {
		var url = Consts.OKRUrl148 + 'api/TradeOrderApi?isOoverview=true&intervalTime=0&authStatus='+ authStatus +'&pageIndex=' + constens.pageIndex + '&pageSize=' + constens.pageSize;
		mui.ajax(url, {
			data: {},
			...
			success: function(data) {
				...
				if(data.code == 0) {
					if(data.bodyMessage) {
						...
						$(dataMsg.PageDatas).each(function(i, item) {
							/*格式化平台及平台图片*/
							if(item.TradeBroker.length > 0) {
								for(var y = 0; y < constens.platformData.length; y++) {
									if(constens.platformData[y].platform == item.TradeBroker) {
										dataMsg.PageDatas[i].TradeBroker = constens.platformData[y].headimg;
										break;
									}
								}
							}
						...
						if(!empty){
							...
							if(empty < 0){
								...
								vm.sm = vm.sm.concat(dataMsg.PageDatas);
								constens.totalPage = dataMsg.TotalPages;
								if(constens.pageIndex >= constens.totalPage) {
