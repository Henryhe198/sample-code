...
require(['vue', 'molieInit', 'IsLoadMethod'], function (vue, init, IsLoadMethod) {
    //微信弹层
	...
	var WinUrlId = GetQueryString('id'),
		UsercurId = GetQueryString('curuser') || 0,
		UserUrlid = GetQueryString('user') || 0;
	...
	var vm = new vue({
		...
		var url = Consts.ForumApi + 'api/BBSPostedNormalApi?ShareId=' + WinUrlId + '&SharecurUserId=' + UsercurId + '&SharepUserId=' + UserUrlid;
		mui.ajax(url, {
			data: {},
			...
			success: function(data) {
				if(data.code == 0) {
					if(data.bodyMessage && data.bodyMessage != "null") {
						...
						var replyCon = dataCen.Reply.PageDatas;
						var replyReg = /\{img[\d]\}/g;
						...
						if(dataMP.Title && dataMP.Time && dataMP.NickName && dataMP.Mcontent) {
							html += '<div class="ct-name-time"><span>' + Time_Month + '月' + Time_Date + '日' + '</span><span>' + dataMP.NickName + '</span></div>' + '<h2 class="ct-title">' + dataMP.Title + '</h2>';
						...
						//评论加载
	                    for (var t = 0; t < replyCon.length; t++) {
	                    	var replyMain = JSON.parse(replyCon[t].MContent).Content;
	                    	var replySrc = JSON.parse(replyCon[t].MContent).Url;
	                    	comment +='<div class="comment_read"><div class="comment_img"><img src="' + replyCon[t].UserHeadImg + '"></div>';
	                    	comment +='<div class="comment"><h3 class="name">'+ replyCon[t].UserName + '</h3>';
	                    	if(replyReg.test(replyMain)){
	                    		replyMain = replyMain.replace(replyReg,function(str){
	                    			for(var i=0;i<replySrc.length;i++){
	                    				if(replySrc[i].Key == str){	                    					
	                    					return '<img src="' + replySrc[i].SmallPic + '"data-preview-src="" data-preview-group="1">'
	                    				}
	                        ...
	                        comment += '<p class="activeshow"><span>' + replyMain + ' </span></p><div class="commetn_icon_show"><i class="mui-icon mui-icon-arrowdown"></i></div></div></div>'
