...
define("touchs", function(){});

define('ImageMethod',['molieInit','touchs'], function (m) {
...
return {
	...
	echo.init();
	require(['ImageMethod', 'imgloads'], function (im, imgLoad) {
	    //var PullY = new IScroll('#wrapscroll', { probeType: 3, mouseWheel: true, bounce: false });
	    var WinUrlId = GetQueryString("id");

	    //跳转链接
	    $('body').on('tap', '.Related_Centent', function () {
	        $this = $(this);
	        window.location.href = '/News/NewsTemplate?id=' + $this.data('id');
	    })
	    $(function () {
	        $('#fh_close').on('click', function () {
	            //var url = Consts.H5Domain + 'Preview/IndustryNews?previewArrNewId=' + WinUrlId;
	            //var url = 'http://apph5.fx001.com/Preview/IndustryNews?previewArrNewId=' + WinUrlId;
	             location.href = document.referrer;
	            //$('#fh_close').attr('href', url);
	        })
	    })
	    var myscroll = new IScroll("#wrapscroll", { hScrollbar: false, vScrollbar: false });
	    $.ajax({
	        type: 'GET',
	        url: Consts.OKRUrl148 + 'api/NewsMobileApi?newsId=' + WinUrlId,
	        ...
	        success: function (data) {
	            ...
	            if (data.code == 0) {
	                if (data.bodyMessage && data.bodyMessage != "null") {
	                    ...
	                    if (dataCen.MContent && dataCen.Title) {
	                        html += '<div id="title">' + dataCen.Title + '</div>';
	                        if (dataCen.AddTime) {
	                            html += '<div class="rich_meta"><em>' + dataCen.AddTime + '</em></div>';
	                        }
	                        var DateContent = dataCen.MContent.Content.replace(/\n/g, '<br/><br/>');

	                        if (dataCen.MContent.Url) {
	                            var UrlObj = dataCen.MContent.Url;
	                            if (UrlObj.length > 0) {
	                        ...
	                        html += '<div id="main_content">' + DateContent + '</div>';
	                        //console.log(Content);
	                        // $('#' + Content).html(html);
	                    ...
	                    if (dataCen.More) {
	                        var DataMore = dataCen.More;
	                        for (var i = 0; i < DataMore.length; i++) {
	                            News += '<div class="Related_Centent" data-id="'+ DataMore[i].NewsId + '">';
	                            News += '<a href="/News/NewsTemplate?id=' + DataMore[i].NewsId + '">';
	                            News += '<div class="Related_Centent_img"><img src="' + DataMore[i].TitleImages.Small + '" /></div>';
	                            News += '<div class="Related_Centent_text">' + DataMore[i].Title + '</div></a>';
	                            News += '</div>';
	                        }
	                        document.getElementById('Related_Centent_id').innerHTML = News;
	                    }
	                    ...
	                    setTimeout(function () {
	                        // 判断图片加载状况，加载完成后回调
	                        imgLoad.isImgLoad($img, function () {
	                            myscroll.refresh();
	                        });
	                    }, 500)
	                    im.ImgInit({
	                        id: 'page_content',
	                        MethodName: 'ImagePagerStringPath'
	                    });
	                }
	                else {
	                    alert('暂无内容');
...
;
define("DetailsTemplateNews", function(){});
