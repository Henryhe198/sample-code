...
require(['vue', 'IsLoadMethod','molieInit'], function (vue, IsLoadMethod,init) {
	...
	$.ajax({
		type: 'GET',
		url:Consts.FXUrl + '/api/RightsConsultationApi/' + consultId,
		data: {},
		cache:true,
		dataType: 'json',
    ...
    function getComments() {
    	var url = Consts.FXUrl + '/api/CommentMobileApi?type=%E5%92%A8%E8%AF%A2&aboutId=' + consultId + '&scoreType=%E5%85%A8%E9%83%A8&pageIndex=1&pageSize=3';
        mui.ajax(url, {
			data: {},
			...
			success: function(data) {
				if(data.code == 0) {
					if(data.bodyMessage) {
						...
						if(dataMsg.PageDatas.length > 0){
							vm.comment = dataMsg.PageDatas;
						}else{
							vm.comment = "no";
						}
...
define("consultTemplate", function(){});
