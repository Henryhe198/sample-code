...
require(['vue', 'booksMethod'], function(vue, bk) {
	...
	var blank = [{
			...
			"banktype": 1,
			"data": "",
			"dataUrl": '../../js/gulp/JSON/BankNationwide.json',
		}, {
...

		{
			...
			"banktype": 1,
			"data": "",
			"dataUrl": '../../js/gulp/JSON/BankSingle.json',
		}, {
			"showBankDom": document.querySelector('#showUserPicker3'),
			"bankIdDom": document.querySelector('#bankId3'),
			"title": "选择城市",
			"banktype": 4,
			"data": "",
			"dataUrl": Consts.OKRUrl148 + 'api/BankSwiftCodeApi?bankId='
		}, {
			"showBankDom": document.querySelector('#showUserPicker4'),
			"bankIdDom": document.querySelector('#bankId4'),
			"title": "选择银行",
			"banktype": 1,
			"data": "",
			"dataUrl": '../../js/gulp/JSON/BankInvestment.json',
		}, {
			"showBankDom": document.querySelector('#showUserPicker5'),
			"bankIdDom": document.querySelector('#bankId5'),
			"title": "选择城市",
			"banktype": 4,
			"data": "",
			"dataUrl": Consts.OKRUrl148 + 'api/BankSwiftCodeApi?bankId='
	...
	function SelectCallBack(items, itemd) {
		switch(parseInt(TopSwitch.BlankContent.index)) {
			case 0:
				...
				//					Nationwide.city = items.code;
				//				}
				break;
			case 1:
				if(banktype == 1) {
					Place.blank = items.bankId;
					Place.city = -1;
					$('#bankId3').text('请选择城市');
				} else if(banktype == 4) {
					Place.city = items.code;
				}
				break;
			case 2:
				if(banktype == 1) {
					Foreign.blank = items.bankId;
					Foreign.city = -1;
					$('#bankId5').text('请选择城市');
				} else if(banktype == 4) {
					Foreign.city = items.code;
				}
				break;
			default:
				break;
	...
	var BlankData = {
		...
		//地方银行和外资银行的城市
		OtherFn: function(_this) {
			if(!BlankQuery.CityQuery()) return false;
			var bankid;
			$('.load-index').css('display', 'block');
			if(parseInt(TopSwitch.BlankContent.index) == 1) {
				bankid = Place.blank;
			} else {
				bankid = Foreign.blank;
			}
			var data, bodyMessage;
			$.ajax({
				url: blank[$(_this).attr('id').replace('showUserPicker', '')].dataUrl + bankid,
				//data:,
				//async: false,
				type: "GET",
				dataType: "JSON",
		...
		BankType: function(_this) {
			...
			switch(parseInt(banktype)) {
				case 1:
					this.NationwideFn(_this);
					break;
				case 2:
					this.PlaceFn(_this);
					break;
				case 3:
					this.ForeignFn(_this);
					break;
				case 4:
					this.OtherFn(_this);
				default:
	...
	$('#Inquiry').on('click', function() {
		if(vm.onLine) {
			switch(parseInt(TopSwitch.BlankContent.index)) {
				case 0:
					$('.load-index').css('display', 'block');
					InquiryTypeFn.NationwideFn();
					break;
				case 1:
					$('.load-index').css('display', 'block');
					InquiryTypeFn.PlaceFn();
					break;
				case 2:
					$('.load-index').css('display', 'block');
					InquiryTypeFn.ForeignFn();
					break;
				default:
					return false;
					break;
	...
	var InquiryTypeFn = {
		//全国银行
		NationwideFn: function() {
			if(Nationwide.blank == -1 || Nationwide.capital == -1 || Nationwide.city == -1) {
				mui.alert('请先选择银行、省份、城市！');
				$('.load-index').css('display', 'none');
				return false;
			} else {
				this.AjaxFn(Consts.OKRUrl148 + 'api/BankSwiftCodeApi?bankId=' + Nationwide.blank + '&provinceId=' + Nationwide.capital + '&areaId=' + Nationwide.city);
			};
		},
		// 地方银行
		PlaceFn: function() {
			if(Place.blank == -1 || Place.city == -1) {
				mui.alert('请先选择银行、城市！');
				$('.load-index').css('display', 'none');
				return false;
			} else {
				this.AjaxFn(Consts.OKRUrl148 + 'api/BankSwiftCodeApi?bankId=' + Place.blank + '&provinceId=0&areaId=' + Place.city);
			};
		},
		//外资银行
		ForeignFn: function() {
			if(Foreign.blank == -1 || Foreign.city == -1) {
				mui.alert('请先选择银行、城市！');
				$('.load-index').css('display', 'none');
				return false;
			} else {
				this.AjaxFn(Consts.OKRUrl148 + 'api/BankSwiftCodeApi?bankId=' + Foreign.blank + '&provinceId=0&areaId=' + Foreign.city);
			};
		},
		//ajax方法
		AjaxFn: function(Url) {
	...
	function blankparrten(blank, callback) {
		...
		for(var ii = 0; ii < blank.length; ii++) {
			if(!!blank[ii].showBankDom) {
				$(blank[ii].showBankDom).on('click', function() {
	...
	var TopSwitch = (function() {
		if($('#segmentedControl').children('div')) {
			var BlankContent = {
				...
				//			BlankContent.index = $(this).index();
				//			$('#wrapscroll').children().eq(BlankContent.index).show().siblings().hide();
				//		})
		}
	...
	$('.swift_title').on('tap', function() {
		var arrow = $('.arrow');
		if(arrow.hasClass('p_active')) {
			arrow.removeClass('p_active');
		} else {
			arrow.addClass('p_active');
		}
		$('.swift_ct').toggle();
		YScroll.refresh();
	});
	...
	$('#slider').on('dragstart', function() {
		for(var ii = 0; ii < blank.length; ii++) {
			if(!!blank[ii].showBankDom) {
				$(blank[ii].showBankDom).off('click');
			}
		}
	});
