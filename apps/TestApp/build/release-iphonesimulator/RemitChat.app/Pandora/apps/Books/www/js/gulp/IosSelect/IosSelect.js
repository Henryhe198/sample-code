(function() {
	var HTMLCSS = document.createElement('link');
	HTMLCSS.rel = "stylesheet";
	HTMLCSS.href = "/Stone.HuiXin.H5/js/gulp/IosSelect/IosSelect.min.css";
	document.body.appendChild(HTMLCSS);
	...
	Layer.prototype = {
		...
		bindEvent: function() {
			...
			for (var i = 0, len = sureDom.length; i < len; i++) {
				sureDom[i].addEventListener('tap', function(e) {
					self.close();
				});
			}
			for (var i = 0, len = closeDom.length; i < len; i++) {
				closeDom[i].addEventListener('tap', function(e) {
					self.close();
				});
			}
		},
	...
	IosSelect.prototype = {
		init: function() {
			...
			this.selectTwoObj = {};
			this.selectThreeObj = {};
			this.setOneLevel(this.options.oneLevelId, this.options.twoLevelId, this.options.threeLevelId,this.options.id,this.options.value);
		},
		initLayer: function() {
			var self = this;
			var all_html = [
				...
				'<h2 id="iosSelectTitle" style="margin-top:14px;"></h2>',
			...
			this.selectBtnDom.addEventListener('tap', function(e) {
		...
		setOneLevel: function(oneLevelId, twoLevelId, threeLevelId,showid,showname) {
			var oneLevelData = this.getOneLevel();
			if (!oneLevelId) {
				oneLevelId = oneLevelData[0][showid];
			}
			var oneHtml = '';
			var self = this;
			var atIndex = 0;
			...
			oneLevelData.forEach(function(v, i, o) {
				if (v[showid] === oneLevelId) {
					oneHtml += '<li ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[showname]+ '</li>';
					atIndex = i + 1 + 3;
				} else {
					oneHtml += '<li ' + iosSelectUtil.attrToHtml(v) + '>' + v[showname] + '</li>';
				}
			});
			oneHtml += '<li></li>';
			oneHtml += '<li></li>';
