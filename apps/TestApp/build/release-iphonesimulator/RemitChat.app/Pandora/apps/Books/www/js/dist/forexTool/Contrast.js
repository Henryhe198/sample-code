...
require(['booksMethod'], function(bk) {
	...
	if(mui.os.ios){
		...
		$('#siftings').css({'top':'20px'});
		$(".ct-scroll").css({"margin-top":"64px"});
		$('#popup').css({'padding-top':'44px'});
		Height = 0;
	}
	$('body').show();
		//设置高度
	$('#popup').height($(window).height() - Height);

	var Url = document.URL;

	$('#btn').on('tap',function(){ 
		if(spreadObj.onLine) {
			$('#btn').addClass('PIPS').siblings().removeClass('PIPS');
			var NumberTwo = Url + '?2';
			var Type = NumberTwo.slice(NumberTwo.lastIndexOf('?') + 1);
			CallTtype(Type);
		} else {
			mui.toast('网络未连接，请检查网络！');
		}
	...
	$('#sum').on('tap',function(){ 
		if(spreadObj.onLine) {
			$('#sum').addClass('PIPS').siblings().removeClass('PIPS');
			var NumberOne = Url + '?1';
			var Type = NumberOne.slice(NumberOne.lastIndexOf('?') + 1);
			CallTtype(Type);
		} else {
			mui.toast('网络未连接，请检查网络！');
		}
	...
	function CallTtype(Type) //根据链接获判定是点差还是报价     1为 点差    2 为报价
	{
		//var Type = Url.slice(Url.lastIndexOf('/') + 1);
		switch(parseInt(Type)) {
			case 1:
				spreadObj.isContrast = false;
				return false;
			case 2:
				spreadObj.isContrast = true;
				return true;
			default:
				return null;
				break;
	...
	//var tradesAll = [],//交易品种总量
	//trades = [],//交易品种默认
	//tradersAll = [],//交易商总量
	//traders = [],//交易商默认排序
	//dataMsg = [];//数据对应库
	//isContrast = false;//判定是点差还是报价 默认为点差
	var spreadObj = {
		tradesAll: [], //交易品种总量
		trades: [], //交易品种默认
		tradersAll: [], //交易商总量
		traders: [], //交易商默认排序
		dataMsg: [], //数据对应库
		isContrast: CallTtype(), //判定是点差还是报价 默认为点差    
		ZeroHourAryPZ: [], //临时存储品种
		ZeroHourAryForex: [], //临时存储交易商
		Url: Consts.NewsPush,
		onLine: true,
	}
	...
	//判断是否有网络
	onLine(function() {
		spreadObj.onLine = true;
	}, function() {
		spreadObj.onLine = false;
	})
	var SignalrFn = (function() {
		var connection = $.hubConnection(Consts.singnalrUrl + '/signalr');
		var connHunProxy = connection.createHubProxy('MT4Hub');
		$('.load-index').css('display', 'block');
		$.ajax({
			//url: "/ForexTool/GetQuoteTrader",
			url: spreadObj.Url + '/api/FX_QuoteTraderApi',
			type: "GET",
			dataType: "json",
			//async: false,
			success: function(data) {
				...
				var data = JSON.parse(data.bodyMessage);
				if(!data) {
					layer.msg("暂无请求数据");
					return false;
				}
				for(var i = 0; i < data.length; i++) {
					spreadObj.tradersAll.push(data[i].PlatName);
					spreadObj.ZeroHourAryForex.push(data[i].PlatName);
					spreadObj.traders.push(data[i].PlatName);
				}
				var TradingModels = data[1].TradingModels;
				for(var o = 0; o < TradingModels.length; o++) {
					spreadObj.tradesAll.push(TradingModels[o].UnifieldName);
					spreadObj.ZeroHourAryPZ.push(TradingModels[o].UnifieldName);
					spreadObj.trades.push(TradingModels[o].UnifieldName);
				}
				//if (document.URL.indexOf("isContrast") >= 0) {
				//    $(".top_title h3 a").eq(1).addClass("cur");
				//    ContrastFn(true);
				//} else {
				//    $(".top_title h3 a").eq(0).addClass("cur");
				//    ContrastFn(false);
				//}

				addMenuTrades($("#varieties"), spreadObj.trades, spreadObj.tradesAll);
				addHtmlTrades($("#tradesName"), spreadObj.trades);
				addMenuTrades($("#traders"), spreadObj.traders, spreadObj.tradersAll);

				//			WidthRefresh();

				spreadObj.dataMsg = data;

				connHunProxy.on('ReceiverQuote', function(json) {
					//console.log(json);
					//AddPageAdsJson(json,dataMsg,spreadObj.trades,traders)
					//    // Add the message to the page.
					setTimeout(function() {
						AddPageAdsJson(json, spreadObj.dataMsg, spreadObj.trades, spreadObj.traders, spreadObj.isContrast)
					}, 100);
				});
				//// 开始连接
				connection.start({
					...
					connHunProxy.invoke('RegisterClient', '3');
				});
	...
	//读写数据
	function AddPageAdsJson(data, dataMsg, trades, traders, isContrast) {
		var data = selJson(data, dataMsg, trades);
		//data = spreadx10(data);//特殊处理GKFX的XAU/USD（处理方式点差x10）
		var contData = isContrast ? data[3] : data[5];
		//console.log(data);
		var PlatForm = data[0].replace(/\s+/g, "") //去掉名称空格
		if(traders) {
			if(!!$("#" + PlatForm).size()) {
				fillHtml(data, trades, isContrast);
			} else {
				for(var i = 0; i < traders.length; i++) {
					//var PlatForm = traders[i].replace(/\s+/g, "")//去掉名称空格
					if(data[0] == traders[i]) {
						var dataHtml = "";
						dataHtml = '<ul id="' + PlatForm + '">'
						$("#traderName").append('<p id="' + PlatForm + 'Name">' + data[0] + '</p>')
							//dataHtml += '<td class="tLogo"><a href="' + data[7] + '" target="_blank"><img src="' + data[6] + '">' + data[0] + '</a></td>'
						for(var x = 0; x < trades.length; x++) {
							if(data[2] == trades[x]) {
								dataHtml += '<li data-ask="' + data[3] + '">' + contData + '</li>'
							} else {
								dataHtml += '<li>-</li>'
							}
						}
	...
	//数据的判断，给数据加背景色
	function fillHtml(data, trades, isContrast) {
		var contData = isContrast ? data[3] : data[5];
		var _trDom = $(".Right_content").find("ul");
		for(var x = 0; x < trades.length; x++) {
			var tradesName = data[0].replace(/\s+/g, "");
			if(data[2] == trades[x]) {
				$("#" + tradesName).children().eq(x).html(contData); //先把数据写到相应位置
				var maxSpread = {},
					minSpread = {}; //创建两个临时对象存放最大最小的数据及位置（可能是多个）
				_trDom.each(function(index) { //遍历行
					//if (index) {//过滤第一行标题
					var _chilTd = $(this).children("li").eq(x);
					var sp = isNaN(_chilTd.text()) ? -1 : Number(_chilTd.text()); //无数据时赋-1
					_chilTd.removeClass(); //移除当前列所有背景
					if(index == 0) { //第一列时特殊处理（可能只有一个交易商，也就是一行数据）
						sp < 0 ? minSpread.spread = 100 : minSpread.spread = sp; //当前列的第一行数据为空时，赋值给100，方便后面做最小值判断
						maxSpread.spread = sp; //把当前数据赋值给minSpread的spread属性
						maxSpread.trIndex = [index]; //把行的索引值赋给maxSpread的trIndex属性（定义trIndex为数组）
						maxSpread.tdIndex = [x]; //列索引
						minSpread.trIndex = [0]; //第一列数据时把最小值的索引赋0
						minSpread.tdIndex = [x];
					} else if(sp >= maxSpread.spread && sp >= 0) { //当sp=0是为无数据 则不进入if
						if(sp == maxSpread.spread) { //当等于时追加进相关数组
							maxSpread.spread = sp;
							maxSpread.trIndex.push(index);
							maxSpread.tdIndex.push(x);
						} else { //当不等于时当成唯一最大数赋值给对象属性
							maxSpread.spread = sp;
							maxSpread.trIndex = [index];
							maxSpread.tdIndex = [x];
						}

					} else if(sp <= minSpread.spread && sp >= 0) {
						if(sp == minSpread.spread) {
							minSpread.spread = sp;
							minSpread.trIndex.push(index);
							minSpread.tdIndex.push(x);
						} else {
							minSpread.spread = sp;
							minSpread.trIndex = [index];
							minSpread.tdIndex = [x];
						}
					}
				...
				for(var y = 0; y < maxSpread.trIndex.length; y++) { //遍历对象的trIndex属性
					if(maxSpread.tdIndex[0] >= 0) { //当trIndex，tdIndex属性第一项都不等于0时
						_trDom.eq(maxSpread.trIndex[y]).children("li").eq(maxSpread.tdIndex[y]).removeClass().addClass("red"); //按索引查找DOM并添加背景
					}
				}
				for(var y = 0; y < minSpread.trIndex.length; y++) {
					if(minSpread.tdIndex[0] >= 0) {
						_trDom.eq(minSpread.trIndex[y]).children("li").eq(minSpread.tdIndex[y]).removeClass().addClass("green");
					}
				}
			}
		}
	...
	////每次都重新设置下宽度，重置拖动效果
	//function WidthRefresh() {
	//	var width = (10 * parseInt($('#tradesName').children().length)) + 'rem';
	//	var widths = (3.8 * parseInt($('#tradesName').children().length)) + 'rem';
	//	$('#scroll1').width(widths);
	//	$('#tradesName').width(width);
	//	$('.Right_content').width(width);
	//}
...
////			Img.className = 'img_up';
////			document.getElementById('siftings').appendChild(Img);
		} else{
			$('#main').show();
			$('#popup').hide();
			$('.img_up').hide();
	...
	// 判断是否全选
	// type   1是品种的判断   2是交易商的判断
	function isSelected(type, SlectDom) {
		switch(type) {
			case num.selectType.PZ:
				if(spreadObj.ZeroHourAryPZ.length == spreadObj.tradesAll.length) {
					SlectDom.addClass(num.cls.st);
				} else {
					SlectDom.removeClass(num.cls.st);
				}
				break;
			case num.selectType.F:
				if(spreadObj.ZeroHourAryForex.length == spreadObj.tradersAll.length) {
					SlectDom.addClass(num.cls.st);
				} else {
					SlectDom.removeClass(num.cls.st);
				}
	...
	//点击交易品种 执行
	function PZFn(e) {
		var _span = $(e.target)[0].tagName == 'A' ? $(e.target).children() : $(e.target);
		var text = _span.text(); //点击的值
		var Parent = _span.parent().parent(); //父节点
		// console.log(Parent);
		var SlectDom = Parent.children('a:first').children();

		//点击某个品种
		function ChildrenClick() {
			if(_span.hasClass(num.cls.ch)) {
				_span.removeClass(num.cls.ch);
				spreadObj.ZeroHourAryPZ.remove(text);
				Parent.removeClass(num.cls.st);
				isSelected(num.selectType.PZ, SlectDom);
			} else {
				_span.addClass(num.cls.ch);
				spreadObj.ZeroHourAryPZ.push(text);
				isSelected(num.selectType.PZ, SlectDom);
			}
		}
		...
		function ChildrenSelect() {
			...
			if(_span.hasClass(num.cls.st)) {
				Parent.children('a').children().removeClass(num.cls.ch);
				_span.removeClass(num.cls.st);
				spreadObj.ZeroHourAryPZ = [];
			} else {
				Parent.find('a:not(a:first) span').addClass(num.cls.ch);
				_span.addClass(num.cls.st);
				spreadObj.ZeroHourAryPZ = spreadObj.tradesAll.concat();
			}
		}
	...
	function TraderFn(e) {
		...
		function ChildrenClick() {
			if(_span.hasClass(num.cls.ch)) {
				_span.removeClass(num.cls.ch);
				spreadObj.ZeroHourAryForex.remove(text);
				Parent.removeClass(num.cls.st);
				isSelected(num.selectType.F, SlectDom);
			} else {
				_span.addClass(num.cls.ch);
				spreadObj.ZeroHourAryForex.push(text);
				isSelected(num.selectType.F, SlectDom);
			}
		}
		...
		function ChildrenSelect() {
			if(_span.hasClass(num.cls.st)) {
				Parent.children('a').children().removeClass(num.cls.ch);
				_span.removeClass(num.cls.st);
				spreadObj.ZeroHourAryForex = [];
			} else {
				Parent.children('a').children().addClass(num.cls.ch);
				_span.removeClass(num.cls.ch).addClass(num.cls.st);
				spreadObj.ZeroHourAryForex = spreadObj.tradersAll.concat();
			}
		}
	...
	//交易品种
	$('#varieties').on('tap', function(e) {
		PZFn(e).Init();
	})

	//交易商
	$('#traders').on('tap', function(e) {
		TraderFn(e).Init();
	})

	// 点击重置
	$('#a_cz').on('tap', function() {
		$('#varieties,#traders').find('a:not(a:first) span').addClass(num.cls.ch);
		$('#varieties,#traders').find('a:first span').addClass(num.cls.st);
		spreadObj.ZeroHourAryPZ = spreadObj.tradesAll.concat();
		spreadObj.ZeroHourAryForex = spreadObj.tradersAll.concat();
	})

	//点击确定
	$('#a_action').on('tap', function() {
			if(spreadObj.ZeroHourAryPZ.length < 4) {
				mui.alert("最少选择4个交易品种");
				return false;
			}

			if(spreadObj.ZeroHourAryForex < 1) {
				mui.alert("最少选择1个交易商");
				return false;
			}
			spreadObj.trades = spreadObj.ZeroHourAryPZ.concat();
			spreadObj.traders = spreadObj.ZeroHourAryForex.concat();
			...
			$('.img_down').show();
			//		WidthRefresh();
			//  moblieTypeFn(function () { nativeMethod.screeningSubmit() });
		})
		//function moblieTypeFn(Fn) {
		//  if (typeof (Fn) === 'function') {
		//      switch (moblieType()) {
