...
require(['booksMethod'], function(bk) {
	if(mui.os.ios) {
		$("header").css({"padding-top":"20px","height":"64px"})
		$(".mui-scroll-wrapper").css({"top":"64px"})
	}
	$("body").show();
	
	var id = GetQueryString('id');
	var type = GetQueryString('type');
	var idContent = ["交易商简介", "点差表", "利息 爆仓", "出入金", "代理政策", "模拟账户", "软件下载", "表格下载", "移动客户端", "IOS客户端", "安卓客户端"];
	var str, aurl = Consts.OKRUrl148+'/api/BrokerMoreApi/' + id + '?typeId=' + type,onLineB=true;
	$(".mui-title").html(idContent[type - 1]);
	...
	mui.ajax(aurl, {
		...
		success: function(data) {
			...
			var msgData = eval('(' + data.bodyMessage + ')');
			if(!msgData.length){
				mui.toast("暂无数据");
				return false;
			}
			var str = msgData[0].moreContent;
			var regH = /<(p|div|a|table|tr|thead|td|tbody|h1|h2|h3|h4|h5|h6|span|em|i|strong|ul|li|hr|dl|ol|pre|address|blockquote)[^>]*(style="[^\"\']*[\"\'])>/gi;
			str = str.replace(/(<style[^>]*>[^<]*<\/style>)/gi, '').replace(regH, function(aa, bb, cc) {
				return aa.replace(cc, '');
			});
			document.getElementById("htmlclass").innerHTML = str;

			function ArrayMax(tp) {
				var max = tp[0];
				var len = tp.length;
				for(var i = 1; i < len; i++) {
					if(this[i] > max) {
			...
			if($('table').length > 0) {
				$('table').each(function() {
					var length = $(this).find('tbody').find('tr').eq(0).children().length;
					var width = Dsize * parseInt(length) + 'rem';
					var chwidth = 100 / length + '%';
					ArrayLength.push(length);
					$(this).width(width);
					$(this).find('tr').children().width(chwidth);
				})
			}
			if($('.bg1').length > 0) {
				$('.bg1,.bg2').each(function() {
					var length = $(this).children().length;
					var width = Dsize * parseInt(length) + 'rem';
					var chwidth = 100 / length + '%';
					ArrayLength.push(length);
					$(this).width(width);
					$(this).siblings().width(width);
					$(this).find('.bg1_p1,.bg1_p2').width(chwidth);
					$(this).siblings().find('.bg1_p1,.bg1_p2').width(chwidth);
				})
			}

			MaxLength = ArrayMax(ArrayLength) * Dsize + 'rem';
			$('.mui-scroll').width(MaxLength);

			if(parseInt($('.mui-scroll').width()) < $('body').width()) {

				setTimeout(function() {
					$('.mui-scroll').width($('body').width());
					if($('.bg1,.bg2').length > 0) {
						$('.bg1,.bg2').width($('body').width());
					}
				}, 100);
			} else {
				setTimeout(function() {
