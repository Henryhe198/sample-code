...
require(['vue', 'IsLoadMethod','molieInit'], function (vue, IsLoadMethod, init) {
	...
	var brokerId = GetQueryString('id');
	...
	//资讯tab切换
	$(function(){
		$('.tab-btn li').on('tap',function(){
			$(this).addClass('active').siblings().removeClass('active')
			.parents('.box').find('.main .news-list').eq($(this).index()).show()
			.siblings().hide();						
		})
		
	})
	
	var url = Consts.FXUrl + '/api/BrokerApi?brokerId=' + brokerId;
	mui.ajax(url, {
		data: {},
		...
		success: function(data) {
			...
			if(data.code == 0) {
				if(data.bodyMessage && data.bodyMessage != "null") {
					...
					vm.share = dataMain;
					vm.brokerInfo = dataMain.brokerAddInfo;
	...
	$.ajax({
		type: 'GET',
		url:Consts.FXUrl + '/api/ANNCMobileApi?type=%E4%BA%A4%E6%98%93%E5%95%86%E4%BC%98%E6%83%A0%E6%B4%BB%E5%8A%A8&brokerId=' + brokerId + '&pageIndex=1&pageSize=3',
		data: {},
		cache:true,
		dataType: 'json',
		success: function(data) {
			...
			if(data.code == 0) {
				...
				if(data.bodyMessage && data.bodyMessage != "null") {
                    ...
                    vm.activity = dataMain.PageDatas;
	...
	$.ajax({
		type: 'GET',
		url:Consts.FXUrl + '/api/ANNCMobileApi?type=%E4%BA%A4%E6%98%93%E5%95%86%E5%85%AC%E5%91%8A&brokerId=' + brokerId + '&pageIndex=1&pageSize=3',
		data: {},
		cache:true,
		dataType: 'json',
		success: function(data) {
	...
	$.ajax({
		type: 'GET',
		url:Consts.FXUrl + '/api/ANNCMobileApi?type=%E4%BA%A4%E6%98%93%E5%95%86%E6%B1%87%E8%AF%84&brokerId=' + brokerId + '&pageIndex=1&pageSize=3',
		data: {},
		cache:true,
		dataType: 'json',
		success: function(data) {
...
define("tradersTemplate", function(){});
