  ...
  require(['vue', 'booksMethod'], function(vue, bk) {
	...
	$('body').show();
  	...
  	mui.back = function() {
  		...
  		}else{mui.toast('网络未连接，请检查网络！');}
  	}

  	$.ajax({
  		url: Consts.OKRUrl + 'api/FXFinanceSelicApi',
  		//data:,
  		//async: false,
  		type: "GET",
  		dataType: "JSON",
  		success: function(data) {
  			...
  			if(data.code == 0) {
  				...
  				//把内容缓存起来
  				bodyMessage.forEach(function(e) {
  						DataAry.push(e);
  					})
  					//排序
  				...
  				var ChangesPoints, Area, ImgSrc;
  				for(var ii = 0; ii < DataAry.length; ii++) {
  					// 默认 = -1,
  					// 向下 = 0,
  					// 向上 = 1,
  					// 持平 = 2
//					switch(parseInt(DataAry[ii].ChangesDir)) {
//						case 0:
//							ChangesPoints = '<span class="green">' + DataAry[ii].ChangesPoints + '</span>';
//							break;
//						case 1:
//							ChangesPoints = '<span class="red">' + DataAry[ii].ChangesPoints + '</span>';
//							break;
//						default:
//							ChangesPoints = DataAry[ii].ChangesPoints;
//							break;
//					}

  					//判断是否有国旗图标
  					if(DataAry[ii].AreaImg == "") {
  						Area = DataAry[ii].Area;
  						ImgSrc = 'default';
  					} else {
  						var Img='http://upload.fx110.com/images/nationalflag/';
  						Area = DataAry[ii].Area;
  						DataAry[ii].AreaImg=Img+DataAry[ii].AreaImg;
  					}
  					vm.arr.push(DataAry[ii]);
  				}
