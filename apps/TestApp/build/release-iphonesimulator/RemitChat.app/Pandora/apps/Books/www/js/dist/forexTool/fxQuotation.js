...
require(['vue', 'booksMethod'], function(vue, bk) {
	...
	if(mui.os.ios) {
		...
		$(".ct-select").css("top", "64px");
		$(".mui-content").css("padding-top", "109px");
	}
	$('body').show();
	...
	var bankData = {
		...
		getBankData : function(){
			var that = this;
			var url = Consts.OKRUrl + 'api/FXGlobalApi?id=23';
			mui.ajax(url, {
				data: {},
				...
				success: function(data) {
					if(data.code == 0) {
						if(data.bodyMessage) {
							...
							var arrData = that.setSelectData(dataMsg[0].GlobalEnumValueCollection);
							picker.setData(arrData);
							$(".ct-select").on("tap",function(){
								picker.show(function (selectItems) {
									$(".ct-select-name").text(selectItems[0].text);
									that.getQuoteData(selectItems[0].valId);
								})
							})
						}
					} else {
		...
		getQuoteData : function(id){
			var that = this;
			var url = Consts.OKRUrl + 'api/FXFinanceQuoteApi?bankType=' + id;
			mui.ajax(url, {
				data: {},
				dataType: 'json', //服务器返回json格式数据
				type: 'get', //HTTP请求类型
		...
		setSelectData : function(data){
			...
			for(var i in data){
				if(data.hasOwnProperty(i)){
					var a = {};
					a.id = data[i].EnumId;
					a.typeId = data[i].EnumTypeId;
					a.valId = data[i].EnumValueId;
					a.text = data[i].EnumValue;
					arrData.push(a);
				}
			}
			return arrData;
