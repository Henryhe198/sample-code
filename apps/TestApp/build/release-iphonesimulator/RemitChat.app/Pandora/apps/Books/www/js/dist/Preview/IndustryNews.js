...
//  var num = $('#hiddenId').val();
    var Url=window.location.href;
    var previewArrNewId=Url.slice(Url.lastIndexOf('=')+1);
    
    var ii = 1;
    $.ajax({
        type: 'GET',
        url: Consts.OKRUrl148+ 'api/NewsApi?previewArrNewId=' + previewArrNewId,
        //url: 'http://10.0.0.147:8024/api/NewsApi?previewArrNewId=' + num,
        success: function (data) {
            ...
            var time = timeStamp(datas.AddTime);
            var time_yue = time.slice(5, 7);
            var time_ri = time.slice(8, 10);
            var Bg = '';
            var one = '';
            var rem = [];
            var tem = previewArrNewId.split(",");
            ...
            for (var i = 0; i < Page.length; i++) {
                if (rem.length == ii) {
                    one += '<div class="Title_one">'
                           + '<div class="one_Title" >'
                           + '<a href="' +Page[i].Link+'" class="ifrmae_a">'
