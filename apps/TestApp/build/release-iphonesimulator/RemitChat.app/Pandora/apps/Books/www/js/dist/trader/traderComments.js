...
require(['vue'], function (vue) {
    ...
    mui.init({
    	pullRefresh:{
    		...
    		up: {
				...
				callback: pullUpAction //必选，刷新函数，根据具体业务来编写，比如通过ajax从服务器获取新数据；
			}
    	}
    	
    })

    $(".publish_comment").on("tap", function () {
    	if(vm.onLine){
	    	//打开页面
			mui.openWindow({
    ...
    function getComment(type) {
        $.ajax({
            type: "get",
            url: Consts.OKRUrl148 + 'api/CommentApi?type=7&aboutId=' + contest.tradeUserId + '&scoreType=0&returnReply=true&pageIndex=' + contest.pageIndex + '&pageSize=10',
            dataType: 'json',
            success: function (data) {
            	if (data.code == 0) {
                    if (data.bodyMessage) {
                        dataMsg = eval('(' + data.bodyMessage + ')');
						vm.comment = vm.comment.concat(dataMsg.PageDatas);
						contest.totalPages = dataMsg.TotalPages;
						if(contest.pageIndex >= contest.totalPages){
...
define("traderComments", function(){});
