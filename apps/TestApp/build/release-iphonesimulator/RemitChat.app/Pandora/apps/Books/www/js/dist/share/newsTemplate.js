...
require(['vue', 'IsLoadMethod','molieInit'], function (vue, IsLoadMethod,init) {
	...
	var vm = new vue({
		...
		var url = Consts.OKRUrl + 'api/FXFinanceNewsApi?newsId=' + newsId + '&releaseTime=' + timeVal;
		mui.ajax(url, {
			data: {},
			...
			success: function(data) {
				if(data.code == 0) {
					if(data.bodyMessage && data.bodyMessage != "null") {
						...
						var dateTime = dataMain.CreateTime.slice(11);
						
						if(dataMain.Title && dataMain.CreateTime && dataMain.Content) {
							html += '<div class="ct-name-time"><span>' + dateTime + '</span></div>';
							
							html += '<p>' + dataMain.Content + '</p>';
							vm.share = html;							
