...
$(function () {
    var PullY = new IScroll('#wrapscroll', { probeType: 3, mouseWheel: true, bounce: false });
    //var PullX = new IScroll('#wrapscroll1', { probeType: 3, mouseWheel: true, bounce: false, scrollX: true, scrollY: false });
    //var PullX2 = new IScroll('#wrapscroll2', { probeType: 3, mouseWheel: true, bounce: false, scrollX: true, scrollY: false });
...
//	});
    ...
    //交易商
    function EvaluateFn() {
        //var dizhiUrl = window.location.href;
        //var IDurl = dizhiUrl.split("?id=");
        //var count = IDurl[1];
        var Url=window.location.href;
        var UrlId=Url.slice(Url.lastIndexOf('?')+1);
        var Id=UrlId.split('=')[1];
         var arryId=Id.split("&")[0];
       
//      var arryId = $('#hiddenId').val();
        $.ajax({
            type: 'GET',
          url: Consts.OKRUrl148 + 'api/BrokerEvalPeriodApi?previewPeriodId=' + arryId,
//          url: 'http://10.0.0.147:8024/api/BrokerEvalPeriodApi?previewPeriodId=' + arryId,
            success: function (data) {
                ...
                //var one = '';
                //var rem = [];
                //var tem = winUrl.split(",");
                //for (var n = 0; n < tem.length; n++) {
                //    rem[n] = parseInt(tem[n]);
                //}
                for (var i = 0; i < datas.length; i++) {
                    var Page = datas[i].Content;
                    for (var j = 0; j < Page.length; j++) {
                        //if (rem.length == 1) {
                        //    one += '<div class="Title_one">'
                        //           + '<div class="one_Title" >'
                        //           + '<a href="' + Consts.DistAddress + '/News/NewsTemplate?id=' + Page[i].Id + '" class="ifrmae_a">'
                        //           + '<p class="title_p">' + Page[i].Title + '</p><p class="time_p">' + time_yue + '月' + time_ri + '日' + '</p></a><img src="' + Page[i].TitleImages.Big + '" />'
                        //           + '<p class="nr_p">' + Page[i].Summary + '</p></div>'
                        //    $('.Title_News').html(one);
                        //}
                        if (j == 0) {
                            title += '<div class="max_div">'
                                   + '<div class="img_bg" >'
                                   + '<a href="' + Page[j].Link + '" class="ifrmae_a"><img src="' + Page[j].TitleImages.Big + '" /></a>'
                                   + '<div class="a_width"><a href="' + Page[j].Link + '" class="one_a" >' + Page[j].Title + '</a></div></div>'
                            $('.title_div').html(title);
                        } else {
                            Bg += '<div class="bottom_div">'
                                  + '<a href="' + Page[j].Link + '" class="img_a"><img src="' + Page[j].TitleImages.Big + '" /></a>'
                                  + '<a href="' + Page[j].Link + '" class="nr_div">' + Page[j].Title + '</a><span>' + Page[j].Shared + '</span></div>'
                            $('.one_div').html(Bg);
                        }
                    }
                }
...
//      var arryId = $('#hiddenId').val();
        $.ajax({
            type: 'GET',
        url: Consts.OKRUrl148 + 'api/ANNCApi?previewArrayId=' + arryId,
//        url: 'http://10.0.0.147:8024/api/ANNCApi?previewArrayId=' + arryId,
            success: function (data) {
                var code = data.code;
                switch (parseInt(code)) {
                    case -1:
                ...
                var img = '';
                var Bg = '';
                var Page = datas.Content;
                for (var j = 0; j < Page.length; j++) {
                    if (j == 0) {
                        img += '<div class="max_div">'
                               + '<div class="img_bg" >'
                               + '<a href="' + Page[j].Link + '" class="ifrmae_a"><img src="' + Page[j].TitleImages.Big + '" /></a>'
                               + '<div class="a_width"><a href="' + Page[j].Link + '" class="one_a" >' + Page[j].Title + '</a></div></div>'
                        $('.title_div').html(img);
                        ...
                        Bg += '<div class="bottom_div">'
                              + '<a href="' + Page[j].Link + '" class="img_a"><img src="' + Page[j].TitleImages.Big + '" /></a>'
                              + '<a href="' + Page[j].Link + '" class="nr_div">' + Page[j].Title + '</a></div>'
                        $('.one_div').html(Bg);
                    }
                }
              PullY.refresh();
...
//      var arryId = $('#hiddenId').val();
        $.ajax({
            type: 'GET',
          url: Consts.OKRUrl148 + 'api/SpecialApi?previewArrayId=' + arryId,
//          url: 'http://10.0.0.147:8024/api/SpecialApi?previewArrayId=' + arryId,
            success: function (data) {
                var code = data.code;
                switch (parseInt(code)) {
                    case -1:
                ...
                for (var i = 0; i < Page.length; i++) {
                    if (i == 0) {
                        img += '<div class="max_div">'
                               + '<div class="img_bg" >'
                               + '<a href="' + Page[i].Link + '" class="ifrmae_a"><img src="' + Page[i].TitleImages.Big + '" /></a>'
                               + '<div class="a_width"><a href="' + Page[i].Link + '" class="one_a" >' + Page[i].Title + '</a></div></div>'
                        $('.title_div').html(img);
                    } else {
                        Bg += '<div class="bottom_div">'
                              + '<a href="' + Page[i].Link + '" class="img_a"><img src="' + Page[i].TitleImages.Big + '" /></a>'
                              + '<a href="' + Page[i].Link + '" class="nr_div">' + Page[i].Title + '</a></div>'
                        $('.one_div').html(Bg);
                    }
                }
              PullY.refresh();
    ...
    var urlDizhi = window.location.href;
    var urlSplit = urlDizhi.split('&value=');
    var urlValue = urlSplit[1];
    var Fn = {
        news: 'news',
