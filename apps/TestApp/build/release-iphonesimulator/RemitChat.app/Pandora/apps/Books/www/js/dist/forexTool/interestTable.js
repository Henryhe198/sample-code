...
require(['vue', 'booksMethod'], function(vue, bk) {
	...
	var bankData = {
		...
		getBankData : function(){
			var that = this;
			var url = Consts.OKRUrl + 'api/FXFinanceDepoRateApi?currencyType='+ GetQueryString("currencyType");
			mui.ajax(url, {
				data: {},
				...
				success: function(data) {
					if(data.code == 0) {
	                    ...
	                    vm.interest = dataCent;
	                    vm.currencyName = dataCent[0].CurrencyName;
					}else{
						mui.toast('请求失败');
					}
				},
