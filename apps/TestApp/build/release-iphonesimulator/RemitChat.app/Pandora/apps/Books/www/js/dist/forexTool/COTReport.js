...
require(['vue', 'booksMethod'], function(vue, bk) {
	...
	if(mui.os.ios){
		...
		$(".ct-scroll").css({"margin-top":"64px"});
	}
	$('body').show();
	window.addEventListener("onorientationchange" in window ? "orientationchange" : "resize", function() {
		var win = window;
		//	alert(win.innerHeight + ":" + win.innerWidth);
		var maxwidth = (parseInt(win.innerHeight) > parseInt(win.innerWidth)) ? win.innerHeight : win.innerWidth;
		if(win.orientation == 180 || win.orientation == 0) //竖屏
		{
			//	$('#scroll1').width('65rem');
			if(parseInt($('#scroll1').width()) < parseInt(maxwidth)) {

				setTimeout(function() {
					$('#scroll1').width($('body').width() - $('.Real_left').width())
				}, 100);
			}
		...
		//alert($('.Real_left').width()+":"+$('body').width());
	})

	//	 function MathWidth(){
	//			var Right_tit=(8*parseInt($('.Right_title').children().length))+'rem';
	////			$('#mui_scroll').width(Right_tit);
	////			$('.Right_title').width(Right_tit);
	////			$('.Right_content').width(Right_tit);
	//	}
	...
	$.ajax({
		type: 'GET',
		url: Consts.OKRUrl + 'api/FXFinanceCFTSApi',
		data: '',
		dataType: 'json',
		success: function(data) {
			var datas=JSON.parse(data.bodyMessage);
			var a = datas[0].StartDate.split('0:00:00')[0];
			var time = a.slice(a.indexOf('/'));
			var yue = time.split('/')[1];
			var ri = time.split('/')[2];
			var rq = ri.split(' ')[0];
			var b = datas[0].EndDate.split('0:00:00')[0];
			var times = b.slice(b.indexOf('/'));
			var Xx = times.split('/')[1];
			var Tt = times.split('/')[2];
			var Gg = Tt.split(' ')[0];
			var chushi=yue+'月'+rq+'日';
			var jieshi=Xx+'月'+Gg+'日';
			vm.time.push(chushi,jieshi);
			var Img = 'http://upload.fx110.com/images/nationalflag/';
			for(var i=0;i<datas.length;i++){
				datas[i].SymbolImg=Img+datas[i].SymbolImg;
				vm.arr.push(datas[i]);
			}		
		},
		error: function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			var arr = new Array();
			var arry = new Array();
			var data = eval('(' + data.bodyMessage + ')');
			while(i < data.length) {
				if(data[i].Symbol != '美元') {
					text.push(data[i]);
				}
			...
			for(var j = 0; j < text.length; j++) {
				if((text[j].StartValue < 0) && (text[j].NetChange < 0) && (text[j].EndValue < 0)) {
					arr.push(text[j].NetChange);
				} else if((parseInt(text[j].StartValue) > 0) && (parseInt(text[j].NetChange) > 0) && (parseInt(text[j].EndValue) > 0)) {
					arry.push(text[j].NetChange);
				}

			}
			//取最小净变化数(空头)ps:a-b最小数
			arr.sort(function(a, b) {
				return a - b;
			});
			for(var l = 0; l < data.length; l++) {
				if(data[l].NetChange == arr[0]) {
					var Ss = data[l];
				}
			}
			//取最大净变化数(多头)ps:b-a最大数
			arry.sort(function(a, b) {
				return b - a
			});
			for(var n = 0; n < data.length; n++) {
				if(data[n].NetChange == arry[0]) {
					var Max = data[n];
				}
			}
			var html = "";
			var bodyHtml = Ss.Exponent.split('。')[0];
			var bodyHtmls = Max.Exponent.split('。')[0];
			var HtmlTop = bodyHtml.slice(bodyHtml.indexOf('日') + 1);
			var HtmlBottom = bodyHtmls.slice(bodyHtmls.indexOf('日') + 1);
			var htmls = "<p>" + HtmlTop + '。' + "</p>" +
				"<p>" + HtmlBottom + '。' + "</p>";
			html += htmls;
			$('.Right_bottom').html(html);
		},
		error: function(e) {
			mui.toast('请求超时');
		}
